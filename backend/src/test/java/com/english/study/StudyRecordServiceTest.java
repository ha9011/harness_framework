...
class StudyRecordServiceTest {
    ...
    void getRecords() {
        // given
        StudyRecord r1 = new StudyRecord(2, LocalDate.now());
        StudyRecord r2 = new StudyRecord(1, LocalDate.now().minusDays(1));
        setId(r1, 1L);
        setId(r2, 2L);

        PageRequest pageable = PageRequest.of(0, 10);
        given(studyRecordRepository.findAllByOrderByCreatedAtDesc(pageable))
        ...
        // when
        Page<StudyRecordResponse> result = studyRecordService.getRecords(pageable);
