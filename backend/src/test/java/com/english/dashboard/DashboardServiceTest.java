...
class DashboardServiceTest {
    ...
    void counts() {
        given(wordRepository.countByDeletedFalse()).willReturn(10L);
        given(patternRepository.countByDeletedFalse()).willReturn(5L);
        given(generatedSentenceRepository.count()).willReturn(20L);
        given(reviewLogRepository.findDistinctReviewDates()).willReturn(Collections.emptyList());
        given(reviewItemRepository.countTodayRemaining(eq("WORD"), any(LocalDate.class))).willReturn(0L);
        given(reviewItemRepository.countTodayRemaining(eq("PATTERN"), any(LocalDate.class))).willReturn(0L);
        given(reviewItemRepository.countTodayRemaining(eq("SENTENCE"), any(LocalDate.class))).willReturn(0L);
        given(studyRecordRepository.findTop5ByOrderByCreatedAtDesc()).willReturn(Collections.emptyList());
    ...
    void streak_today() {
        ...
        // 오늘, 어제, 그제 연속 복습
        given(reviewLogRepository.findDistinctReviewDates())
    ...
    void todayReviewRemaining() {
        given(wordRepository.countByDeletedFalse()).willReturn(0L);
        given(patternRepository.countByDeletedFalse()).willReturn(0L);
        given(generatedSentenceRepository.count()).willReturn(0L);
        given(reviewLogRepository.findDistinctReviewDates()).willReturn(Collections.emptyList());
        given(reviewItemRepository.countTodayRemaining(eq("WORD"), any(LocalDate.class))).willReturn(5L);
        given(reviewItemRepository.countTodayRemaining(eq("PATTERN"), any(LocalDate.class))).willReturn(3L);
        given(reviewItemRepository.countTodayRemaining(eq("SENTENCE"), any(LocalDate.class))).willReturn(8L);
        given(studyRecordRepository.findTop5ByOrderByCreatedAtDesc()).willReturn(Collections.emptyList());
    ...
    void recentStudyRecords() {
        ...
        StudyRecord r1 = new StudyRecord(3, LocalDate.now());
        StudyRecord r2 = new StudyRecord(2, LocalDate.now().minusDays(1));
        setId(r1, 1L);
        setId(r2, 2L);
        given(studyRecordRepository.findTop5ByOrderByCreatedAtDesc()).willReturn(List.of(r1, r2));
